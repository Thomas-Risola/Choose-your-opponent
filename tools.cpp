...
void S_kN(std::vector<int>& S,VectorTree* tree,int k,VectorTree* QOmega,const ComparePlayers& comp,const std::vector<std::vector<double>>& probability_matrix, const std::vector<std::vector<bool>>& play_matrix,bool greedy) {
...
bool verif_constraints(const int current_player,const int chosen_player,const std::vector<int>& XN,const std::vector<std::vector<bool>>& play_matrix) {
...
std::vector<double> opponent_choice_optimization_algorithm_rec(std::vector<int>& XN1,std::vector<int>& XN2,std::vector<int>& X1,std::vector<int>& X2,const std::vector<int>& ranking,const VectorTree* QOmega,const std::vector<int>& XN,const std::vector<std::vector<double>>& probability_matrix,const std::vector<std::vector<bool>>& play_matrix,bool greedy);
std::vector<double> maximization_function_optimization_algorithm(
        std::vector<int>& XN1,std::vector<int>& XN2,std::vector<int>& X1,std::vector<int>& X2,
        const std::vector<int>& ranking,const VectorTree* QOmega,const std::vector<int>& XN,
        const int current_player,const int ind_curr_player,
        const std::vector<std::vector<double>>& probability_matrix,const std::vector<std::vector<bool>>& play_matrix) {
...
std::vector<double> maximization_function_greedy_algorithm(
    ...
    for (unsigned int j=1;j<ranking.size();j++)
        if (rnk_chos_player!=j)
            new_ranking.push_back(ranking.at(j)); // Contruction du ranking sans les joueurs déjà fixés
    X2.push_back(chosen_player); // Construction de la solution
    std::vector<double> qXmax=opponent_choice_optimization_algorithm_rec(XN1,XN2,X1,X2,new_ranking,QOmega,XN,probability_matrix,play_matrix,true);
    X2.pop_back(); // Restauration de l'intégrité de X2
    return qXmax;
}
...
std::vector<double> recursive_step_opponent_choice_optimization_algorithm_rec(
        std::vector<int>& XN1,std::vector<int>& XN2,std::vector<int>& X1,std::vector<int>& X2,
        const std::vector<int>& ranking,const VectorTree* QOmega,const std::vector<int>& XN,
        const std::vector<std::vector<double>>& probability_matrix,const std::vector<std::vector<bool>>& play_matrix,bool greedy) {
    if(ranking.size() == 0)
        return std::vector<double>(XN.size(),0);
    ...
    std::vector<double> qX;
    if (greedy)
        qX=maximization_function_greedy_algorithm(XN1,XN2,X1,X2,ranking,QOmega,XN,current_player,ind_curr_player,probability_matrix,play_matrix);
    else
...
std::vector<double> opponent_choice_optimization_algorithm_rec(std::vector<int>& XN1,std::vector<int>& XN2,
                    ...
                    const std::vector<int>& XN,const std::vector<std::vector<double>>& probability_matrix,const std::vector<std::vector<bool>>& play_matrix,bool greedy) {
...
void setDiagonalVictory(std::vector<std::vector<double>> &M){
